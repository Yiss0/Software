...
export default function AddMedicationScreen() {
  ...
  const handleAddTime = () => {
    ...
    if (schedules.some((s) => s.time === localTime)) {
      // Comprobamos duplicados en hora local
    ...
    setScheduleInput((prev) => ({
      ...prev,
      time: "", // Resetea 'frequencyValue' al default correcto según el tipo
      frequencyValue: prev.frequencyType === "HOURLY" ? 8 : 1,
    }));
  ...
  const handleSave = async () => {
    ...
    if (!patientIdToAddFor) {
      Alert.alert("Paciente no encontrado", "No pudimos encontrar el paciente seleccionado. Selecciónalo e inténtalo de nuevo.");
      return;
    }
    ...
    if (user?.role === "CAREGIVER") {
      Alert.alert(
        ...
        "Los cuidadores no pueden añadir medicamentos. Pide a un usuario con permisos que haga los cambios."
      );
      return;
    }
    ...
    try {
      ...
      const newMed = await apiService.addMedication(
        patientIdToAddFor,
        newMedPayload,
        schedulesInUTC
      ); // Enviamos el array en UTC

      if (newMed && newMed.id) {
        Alert.alert("¡Listo!", "El medicamento se guardó correctamente.");
        router.back();
      } else {
      ...
      Alert.alert(
        "No se pudo guardar",
        "Ocurrió un problema al guardar el medicamento. Intenta de nuevo más tarde."
      );
    } finally {
      setLoading(false);
    }
