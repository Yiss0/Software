...
export async function extractMedicationDetails(userMessage: string): Promise<MedicationDetails | null> {
  const prompt = `Analiza: "${userMessage}". Devuelve solo un JSON con la estructura: {"medication":{"name":"string","dosage":"string"},"schedules":[{"time":"HH:mm","frequencyType":"string"}]}`;

  try {
    const response = await axios.post(GEMINI_URL, {
      contents: [{ parts: [{ text: prompt }] }],
      // Mantenemos solo la configuración estrictamente necesaria para que devuelva JSON
      generationConfig: { 
